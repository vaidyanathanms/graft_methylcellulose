...
MODULE INPPARAMS
  ...
  REAL, PARAMETER :: graftdens = 0.0
  INTEGER, PARAMETER :: graft_opt = 2
  INTEGER, PARAMETER :: key_eps_polygraft = 0
  REAL, PARAMETER :: eps_poly_graft = 0
  REAL, PARAMETER :: eps_graft_graft = 0.8
...
SUBROUTINE FIND_GENERATE_AND_WRITE_PAIRCOEFF()
  ...
  IF(key_eps_polygraft == 1 .AND. graftdens .NE. 0) THEN
     ...
     DO i = 1, natomtypes
        ...
        sig_pg = sqrt(siggraft*sigma(u))
        rc_pg  = sqrt(rcutgraft*rcut(u))
